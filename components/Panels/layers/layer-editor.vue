<template>
  <div class="layer-editor-container">
    <div class="controls">
      <div @click="isModalOpened = !isModalOpened" class="controls-button">
        <img src="@/static/icons/playlist_add-24px.svg" />
        <span>Add New Layer</span>
      </div>
      <p v-if="maxLayersAmountReached" class="alert-text">
        Maximum number of layers reached!
      </p>
      <div @click="clearAllLayers()" class="controls-button">
        <img src="@/static/icons/clear_all-24px.svg" />
    ...
    <div v-if="isModalOpened" class="layer-editor-modal">
      <div class="layer-editor-modal-inner">
        ...
        <button @click="addLayer(layerTypeSelect.id, layerTypeSelect.text)">
          Create
        </button>
      </div>
    </div>
...
export default {
  data() {
    return {
      isModalOpened: false,
      layers: [{ id: 0, name: "Temperature (K)", type: "t" }],
      ...
      layerTypes: [
        ...
        {
          id: "groundice",
          text: "Surface H2O ice layer (kg/m2, 0.5: perennial)"
        },
...
.controls-button {
  ...
  &:hover {
    background-color: var(--interfaceSecondaryHoverColor);
    border-radius: 50px;
    transition: 0.5s;
  }
}
